...
const { getAllMyFriends, removeFriend, addFriend, followUser, unfollowUser, getFriendRequests, acceptFriend, rejectFriend, searchUsers } =
  useSupabaseDatabase();
...
async function onAddFriend(friend) {
  searchTerm.value = ''
  if (friend?.following_status != 'following') {
    followUser(localUser.me.value.id, friend.id);
  }
  await addFriend(friend.id);
  await getFriends();
}

async function onFollowUser(user) {
  if (localUser.me.value) {
    searchTerm.value = ''
    await followUser(localUser.me.value.id, user.id);
  }
}

async function onUnfollowUser(user) {
  if (localUser.me.value) {
    searchTerm.value = ''
    await unfollowUser(localUser.me.value.id, user.id);
  }
}

async function onAccept(userId) {
  await acceptFriend(userId);
  await followUser(localUser.me.value.id, userId);
  await getFriends();
  await getRequests();
}
        ...
        <div v-for="user in searchResults" :key="user.id" 
          ...
          <div class="flex">
            <ButtonTransparentIcon
              v-if="user.following_status == 'not following'"
              @click="onFollowUser(user)"
              icon="i-tabler-star"
            />
            <ButtonTransparentIcon
              v-if="user.following_status == 'following'"
              @click="onUnfollowUser(user)"
          ...
          <div v-if="!(friend.friendship_direction == 'incoming' && friend.friendship_status == 'pending') && !['rejected', 'blocked', 'pending'].includes(friend.friendship_status)" class="flex items-center justify-between p-4 bg-white rounded-lg shadow">
            <div class="flex items-center">
              ...
              <div>
                ...
                >
                <ButtonUsername :label="friend.name" />
                </NuxtLink>
                <p class="user-note">
                  {{ friend.note }}
                </p>
        ...
        <template v-if="showOthers" v-for="friend in otherFriends" :key="friend.id">
          <div v-if="!(friend.friendship_direction == 'incoming' && friend.friendship_status == 'pending') && ['rejected', 'blocked', 'pending'].includes(friend.friendship_status)" class="flex items-center justify-between p-4 bg-white rounded-lg shadow">
            <div class="flex items-center">
              <div class="i-tabler-user text-2xl text-gray-600 mr-3" />
              <div>
                <ButtonUsername :to="{ name: 'u-user', params: { user: friend.name }}" :label="friend.name" />
                <p class="user-note">
                  {{ friend.note }}
                </p>
              </div>
